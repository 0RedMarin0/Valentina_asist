...
from tools.VTranslate import Ui_VTranslate, transla
from tools.Valentina import UiValentina as Val
from tools.Valentina_web import Ui_VBrowser
...
class VBrowser(QtWidgets.QMainWindow, Ui_VBrowser):
    def __init__(self):
        ...
        self.back_btn.setStyleSheet("font-size: 15px;")
        self.back_btn.setText("<")
        self.forward_btn.setFixedSize(25, 25)
        self.forward_btn.setStyleSheet("font-size: 15px;")
        self.forward_btn.setText(">")
        self.refresh_btn.setFixedSize(25, 25)
        self.refresh_btn.setStyleSheet("font-size: 15px;")
        self.refresh_btn.setText("○")
        self.home_btn.setFixedSize(25, 25)
        self.home_btn.setStyleSheet("font-size: 15px;")
        self.home_btn.setText("⌂")
        self.url_area.setMinimumHeight(25)
        self.url_area.setText("https://google.com")
        ...
        self.url_area.returnPressed.connect(self.get_url)

    def get_url(self):
        if "https://" in self.url_area.text() or "http://" in self.url_area.text():
            self.web.load(QUrl(f"{self.url_area.text()}"))
        elif "https://" not in self.url_area.text() and ".com" not in self.url_area.text() or ".ru" not in self.url_area.text():
            a = "http://yandex.ru/search/?text=" + self.url_area.text()
            self.web.load(QUrl(f"{a}"))
        elif "https://" not in self.url_area.text() or "http://" not in self.url_area.text():
            a = "http://" + self.url_area.text()
            self.web.load(QUrl(f"{a}"))
...
class VTranslate_GUI(QMainWindow, Ui_VTranslate):
    ...
    def photo_trans(self):
        msg = QMessageBox()
        msg.setStyleSheet("background-color: black;color:white;font-size: 20px;}")
...
class Valentina_main(QtWidgets.QMainWindow, Val):
    ...
    def but3(self):
        ...
        menu.addAction("VScreenShot", self.screen_shot) # SCREENSHOT
        ...
        # menu.addAction("Vkey")                       # МЕНЕДЖЕР ГОРЯЧИХ КЛАВИШ
    ...
    def screen_shot(self):
        time_0 = time.ctime(time.time()).replace(" ", "_").replace(":", "-")
        im1 = pyautogui.screenshot()
        im1.save(fr"C:\Users\RedMarin\Pictures\Screenshots\{time_0}.png")
        os.system(fr"explorer.exe C:\Users\RedMarin\Pictures\Screenshots")
